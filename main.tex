...
Underwater image formation is degraded by several factors, which causes the
    ...
    detection time, this was also considered in the comparison.
    ...
    have shown a significantly improvement in the marker detection rates and
...
%% SJ: "dissolved particles" does not mean anything. A particle is always
%% SJ: floating. A compound such as salt can "dissolve"
...
\cite{diego}.  Figure \ref{fig:apriltag_pedestal} shows an example of apriltag marker detection
in the ocean.
...
%% SJ: you must put the hardware used for performance comparison (type of CPU,
%% SJ: frequency, amount of memory, ...
...
%% SJ: So what are the detection rates ? Are they over the whole stream,
%% SJ: or only the images with markers ? The latter would IMO be better. In any
%% SJ: case, you really should put an idea of the ratio between "images with
%% SJ: markers" and "images without markers"
...
\begin{figure}[!ht]
    ...
    %% SJ: THese are not detection rates, they are a number of frames with detected
    %% SJ: marker
    %%
    %% SJ: and ... error bars. Oh my god. Error bars !
...
\begin{figure}[!ht]
    ...
    %% SJ: Error bars. Oh my god. Error bars !
\end{figure}

%% SJ: detection times are in what unit ?????
...
%% SJ: this is the only subsection in this section. Create one for the first
%% SJ: part ?
\subsection{Visual servoing performance}

%% SJ: honestly, I would have rewritten the whole as "test of pre-processing
%% SJ: method", mentioning in the test that the initial goal was a comparison
%% SJ: but that the detector without preprocessing just didn't detect anything
%% SJ: on that day
%%
%% SJ: the current text sounds a bit silly.
...
%% SJ: Same scale on both detectors ? If yes, I find that a bit ... weak. you
%% SJ: Given that the performance of the "no preprocessing" detector degrades
%% SJ: a lot less with scale, you could have used a much bigger image.
...
%\IEEEtriggercmd{\enlargethispage{-1000mm}}
\IEEEtriggeratref{4}
